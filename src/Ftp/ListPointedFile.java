...
public class ListPointedFile {
	 ...
	 * @throws IOException
	 */
	public ListPointedFile(boolean isPrintCommmand, String str) throws IOException {
		ftp = new FTPClient();
		arFiles = new ArrayList<String>();
		if (isPrintCommmand) {
			ftp.addProtocolCommandListener(new PrintCommandListener(new PrintWriter(System.out)));
		}
		this.login(ip.getHostAddress(), 21, "FtpUser", "Anjianwei");
	 ...
	 */
	public FTPFile[] ListSubFile(String pathName) throws IOException {
		if (pathName.startsWith("/") && pathName.endsWith("/")) {
			if (pathName.contains(";")) {
				...
				if (this.ftp.changeWorkingDirectory(new String(directory.getBytes("GBK"), "iso-8859-1"))) {
					ListPointedFile tempList = new ListPointedFile(false, directory);
					...
					for (FTPFile file : ftpCurrentFile) {
						if (file.isDirectory()) {
							ListSubFile(
									"/" + pathName.split(";")[0] + "/" + file.getName() + ";" + pathName.split(";")[1]);
						}
					}
				}
			} else {
				...
				if (this.ftp.changeWorkingDirectory(new String(directory.getBytes("GBK"), "iso-8859-1"))) {
					...
					for (FTPFile file : ftpCurrentFile) {
						if (file.isDirectory())
							ListSubFile("/" + file.getName() + ";" + pathName);
					}
				}
			}
